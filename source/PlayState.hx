...
#end
#end
import shaders.BarrelBlurEffect;
...
class PlayState extends MusicBeatState {
	...
	public var modchartShaders:Map<String, FlxRuntimeShader> = new Map<String, FlxRuntimeShader>();
	public var modchartCameras:Map<String, LuaCamera> = new Map<String, LuaCamera>();
	...
	public static var mainmenu:Bool = false;
	...
	public var barrelBlur:BarrelBlurEffect = new BarrelBlurEffect();

	// spacebar mechanic stuff
	var spaceMech:FlxSprite = new FlxSprite();
	var numText:FlxText = new FlxText();
	var statusText:FlxText = new FlxText();
	var numTextTween:FlxTween;
	private var dodging:Bool = false;
	private var timeMax:Float = 4.0;
	private var timeThing:Float = 0;

	final defaultGlitchSpeed:Float = 0.35;
	var defaultGlitchAmount:Float = 0.025;
	...
	static var firstStorySong:String;
	...
	override public function create() {
		...
		glitch.speed = defaultGlitchSpeed;
		...
		var stageData:StageFile = StageData.getStageFile(ClientPrefs.charsStages ? curStage : "");
		...
		if (gfGroup != null && this != null && instance != null)
		...
		if (dadGroup != null && this != null)
			add(dadGroup);

		if (boyfriendGroup != null && this != null)
			add(boyfriendGroup);
		...
		if (!stageData.hide_girlfriend || ClientPrefs.charsStages) {
		...
		if (!ClientPrefs.charsStages) {
			...
			// dad.pixels.applyFilter(dad.pixels, dad.pixels.rect, new Point(), new DropShadowFilter(300, 45));
			startCharacterPos(dad, true);
			dadGroup.add(dad);
			startCharacterLua(dad.curCharacter);

			boyfriend = new Boyfriend(0, 0, SONG.player1);
			// boyfriend.useFramePixels = true;
			// boyfriend.pixels.applyFilter(boyfriend.pixels, boyfriend.pixels.rect, new Point(), new DropShadowFilter(300, 45));
			startCharacterPos(boyfriend);
			boyfriendGroup.add(boyfriend);
			startCharacterLua(boyfriend.curCharacter);
		}
		...
		timeTxt.setFormat(Paths.font("wii.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		/*if(FileSystem.exists("mods/data/" + SONG.song.toLowerCase() + "/modchart.json")){
		...
		scoreTxt.setFormat(Paths.font("wii.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.hideHud;
		add(scoreTxt);

		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "your trash", 32);
		botplayTxt.setFormat(Paths.font("wii.ttf"), 30, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
   ...
   mobileControls.visible = false;
		...
		spaceMech.frames = Paths.getSparrowAtlas("spacebar");
		spaceMech.animation.addByPrefix("idle", "spacebar0", 48, false);
		spaceMech.animation.addByPrefix("glove", "spacebar glove press0", 24, false);
		spaceMech.animation.addByPrefix("sword", "spacebar sword press0", 24, false);
		spaceMech.animation.addByPrefix("press", "spacebar press0", 24, false);
		spaceMech.cameras = [camHUD];
		spaceMech.screenCenter();
		spaceMech.alpha = 0;
		spaceMech.antialiasing = ClientPrefs.globalAntialiasing;
		spaceMech.setGraphicSize(Std.int(spaceMech.width * 1.25));
		add(spaceMech);

		numText.alpha = 0;
		numText.setFormat(Paths.font("wii.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		numText.scrollFactor.set();
		numText.borderSize = 2;
		numText.cameras = [camHUD];
		numText.screenCenter();
		numText.antialiasing = ClientPrefs.globalAntialiasing;
		add(numText);

		statusText.alpha = 0;
		statusText.setFormat(Paths.font("wii.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		statusText.y += numText.height + 4;
		...
		firstStorySong = storyPlaylist[0].toLowerCase();
		trace(Progress.getData("playedTutorial"));

		callOnLuas('onCreatePost', []);

		if(curStage.toLowerCase().contains('swordarea'))
		{
			FlxG.mouse.load(Paths.image("pointer").bitmap);
			FlxG.mouse.visible = true;
			defectedPosterHitbox = new FlxSprite().makeGraphic(185, 275, FlxColor.WHITE);
			defectedPosterHitbox.scale.set(0.85, 0.85);
			defectedPosterHitbox.scrollFactor.set(0.85, 0.85);
			defectedPosterHitbox.alpha = 0.00001;
			add(defectedPosterHitbox);
			defectedPosterHitbox.updateHitbox();
			defectedPosterHitbox.screenCenter();
			defectedPosterHitbox.x -= 900;
			defectedPosterHitbox.y += 625;
		}
		...
		/*if(curStage.contains("ring")){
			screen = new FlxSprite(0, 0);
			screen.makeGraphic(200, 100, FlxColor.RED);
			screen.cameras = [camHUD];
			screen.screenCenter();
			add(screen);

			var s = new ScreenShader();
			screen.shader = s;
		}*/
		/*for (m in members){
				if(m.cameras == [camGame] || m.cameras == [FlxG.camera]){
					if(!Std.isOfType(m, FlxCamera)){
						m.cameras.push(camScreen);
					}
				}
			}
			camScreen.zoom = FlxG.camera.zoom; */
		...
		var glitchshader:ShaderFilter = new ShaderFilter(glitch.shader);

		if (ClientPrefs.shaders && SONG.player2 == 'defected' && curSong.toLowerCase() != 'parasitic' /* too laggy*/) {
			if (camGame.filters != null)
				camGame.filters.push(glitchshader);
			else
				camGame.filters = [glitchshader];

			if (camHUD.filters != null)
				camHUD.filters.push(glitchshader);
			else
				camHUD.filters = [glitchshader];
			trace("did the shader stupid");
		}

		if (!Progress.getData("playedTutorial"))
			openPauseMenu();
		// if(SONG.player2 == "defect")
		// iconP2.shader = glitch.shader;

		if (!ClientPrefs.shaders) { // just to be safe
			FlxG.camera.filters = [];
			camHUD.filters = [];
			camOther.filters = [];
			for (member in members) {
				if (member is FlxSprite) {
					cast(member, FlxSprite).shader = null;
				}
			}
		}
	}
		...
		switch (type) {
			case 0:
				if (!boyfriendMap.exists(newCharacter)) {
					...
					newBoyfriend.shadow.alpha = 0.00001;
					startCharacterLua(newBoyfriend.curCharacter);
					#if sys
					trace('Cached ${newCharacter} in ${FlxMath.roundDecimal(Sys.time() - tmr, 2)} seconds');
					#end
				}
	...
	public function getLuaObject(tag:String, text:Bool = true):FlxSprite {
		...
		if (modchartCharacters.exists(tag))
			return modchartCharacters.get(tag);
	...
	public function startCountdown():Void {
		...
		mobileControls.visible = true;
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer) {
				...
				for (character in modchartCharacters) {
					if (tmr.loopsLeft % character.danceEveryNumBeats == 0
				...
				switch (swagCounter) {
					...
					case 3:
						...
						if (gf.curCharacter.contains('boxing'))
	...
	private function generateStaticArrows(player:Int):Void {
		for (i in 0...4) {
			...
			if (player < 1) {
				if (!ClientPrefs.opponentStrums || curSong.toLowerCase() == 'parasitic')
					targetAlpha = 0;
				else if (ClientPrefs.middleScroll)
					targetAlpha = 0.35;
			}
	...
	/*override function draw()
	{
		...
		var bitmapData = new BitmapData(Std.int(1280 / 4), Std.int(720 / 4), true, FlxColor.TRANSPARENT);
		var cameraData:Sprite = FlxG.cameras.list[0].canvas;
		bitmapData.draw(cameraData);
		screen.pixels = bitmapData;
}*/
	...
	override public function update(elapsed:Float) {
		...
		/*if(curStage.contains("ring")){
		var bmd = new BitmapData(FlxG.width, FlxG.height, true, FlxColor.GREEN);
		bmd.draw(FlxG.camera.canvas);
		cast(screen.shader, ScreenShader).sourceBitmap.input = bmd;
		
	}*/

		if(curStage.toLowerCase().contains('swordarea')){
			if(FlxG.mouse.overlaps(defectedPosterHitbox) && FlxG.mouse.justPressed){
				trace("penis");
				if (Progress.getData('playedSwordplay')
					&& Progress.getData('playedBoxing')
					&& Progress.getData('playedBasketball')
					&& Progress.getData('playedWiik1Remixes')
					&& Progress.getData('playedWiik2Remixes')
					&& !Progress.getData('playedDefected')) {
					Progress.setData(true, 'defectedInMenu');
					FlxG.stage.window.alert("Something has changed", "???");
					System.exit(0);
				}
				else{
					FlxG.stage.window.alert(["that did nothing.", "what are you doing?", "well that was a waste of time", "why would you do that?", 
						"well dont you feel silly, thinking that would do anything", "really dude?"][FlxG.random.int(0, 5)], "Alert");
				}
			}
		}
		...
		/*if (FlxG.keys.justPressed.P)
		{
			var ret:Dynamic = callOnLuas('onPause', [], false);
			if(ret != FunkinLua.Function_Stop) {
				openSubState(new ResortPauseMenu());
			}
	}*/
		...
		glitch.amount = FlxMath.lerp(0, glitch.amount, CoolUtil.boundTo(1 - (elapsed * 3.125 * camZoomingDecay * playbackRate), 0, 1));
		...
		if (generatedMusic && !inCutscene) {
			...
			if (startedCountdown) {
				...
				notes.forEachAlive(function(daNote:Note) {
					...
					if (daNote.copyX)
						daNote.x = strumX + FlxMath.fastCos(angleDir) * daNote.distance;
	...
	function openPauseMenu() {
		...
		if (!Progress.getData("playedTutorial"))
			openSubState(new TutorialPromptSubstate());
		else
			openSubState(new ResortPauseMenu());
		// }
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch (eventName) {
					...
					switch (charType) {
						case 0:
							if (boyfriend.curCharacter != value2) {
								...
								boyfriend.shadow.alpha = 0.00001;
			...
			case 'Toggle Trail':
				var valueDAD:Int = Std.parseInt(value1);
				var valueBF:Int = Std.parseInt(value1);

				if(valueBF == 0 || valueDAD == 0){
					for(member in members){
						if (member is FlxTrail){
							remove(member);
						}
					}
				}

				var trailBF:FlxTrail = new FlxTrail(boyfriend, null, 5, 3, valueBF == 1 ? 0.4 : 0);
				trailBF.color = CoolUtil.dominantColor(iconP1);
				trailBF.blend = BlendMode.ADD;
				addBehindBF(trailBF);
				var trailDAD:FlxTrail = new FlxTrail(dad, null, 5, 5, valueDAD == 1 ? 0.4 : 0);
				trailDAD.color = CoolUtil.dominantColor(iconP2);
				...
				/*case 'Punch Prep':
					...
					FlxTween.tween(statusText, { alpha: 1}, Conductor.stepCrochet / 250, {ease: FlxEase.cubeInOut});
				case 'Punch':
					...
					if(val1 != 0){
						numText.text = Std.string(val1);
						numText.screenCenter();
					}else{
						numText.text = "INFINITY";
						numText.screenCenter();
					}
					statusText.text = "NOW!!!";
					statusText.screenCenter();
					timeMax = val2;
					trace("Started punch");
					spaceMech.alpha = 1; //do this just in case..
					numText.alpha = 1;
					statusText.alpha = 1;
					numText.color = FlxColor.WHITE;
					statusText.color = FlxColor.WHITE;
					new FlxTimer().start((Conductor.crochet/1000)*val2,punch,val1);
					new FlxTimer().start(((Conductor.crochet/1000)*val2)*(val1),fade,1); */
		}
		callOnLuas('onEvent', [eventName, value1, value2]);
	}

	private function fade(_):Void {
		FlxTween.tween(spaceMech, {alpha: 0}, Conductor.stepCrochet / 125, {ease: FlxEase.cubeInOut, startDelay: Conductor.stepCrochet / 125});
		FlxTween.tween(numText, {alpha: 0}, Conductor.stepCrochet / 125, {ease: FlxEase.cubeInOut, startDelay: Conductor.stepCrochet / 125});
		FlxTween.tween(statusText, {alpha: 0}, Conductor.stepCrochet / 125, {ease: FlxEase.cubeInOut, startDelay: Conductor.stepCrochet / 125});
	}
	...
	private function punch(_):Void {
		...
		if (numText.text != "INFINITY") {
			numText.text = Std.string(Std.parseInt(numText.text) - 1);
		}
		numText.screenCenter();
		numText.color = FlxColor.RED;
		statusText.text = "NOW!!!";
		statusText.screenCenter();
		statusText.color = FlxColor.RED;
		if (numTextTween != null) {
			numTextTween.cancel();
		}
		numText.scale.x = 1.075;
		numText.scale.y = 1.075;
		numTextTween = FlxTween.tween(numText.scale, {x: 1, y: 1}, 0.2, {
			onComplete: function(twn:FlxTween) {
				numTextTween = null;
				numText.color = FlxColor.WHITE;
			}
		...
		if (ret != FunkinLua.Function_Stop && !transitioning) {
			if (SONG.validScore) {
				...
				/*var data:String = Json.stringify({
					song: SONG.song,
					score: songScore,
					accuracy: FlxMath.roundDecimal(percent, 4) * 100,
					stage: curStage,
					char: SONG.player1
				}, null, "\t");
				trace("saving title data shit lol");
				File.saveContent("titleReplay.json", data + "\n");
				trace(data); */
				#end
			}
			playbackRate = 1;
			...
			if (isStoryMode) {
				...
				if (storyPlaylist.length <= 0) {
						...
						switch (firstStorySong) { // you cant use arrays in switch cases???
							...
							case 'boxing-match-resort' | 'boxing match resort':
								...
								/*if (Progress.getData('playedSwordplay')
									...
									&& !Progress.getData('playedDefected')) {
									Progress.setData(true, 'defectedInMenu');
									// Dialogs.message("???", "Something is defecting...\nin the menu...", true);
								} else {*/
									FlxG.switchState(() -> new StoryMenuState());
								//}
						}
					...
					if (mainmenu) {
						PlayState.SONG = Song.loadFromJson(PlayState.storyPlaylist[0] + '-' + MainMenuState.diff, PlayState.storyPlaylist[0]);
						if (MainMenuState.diff == 'normal')
	...
	public function spawnNoteSplashOnNote(note:Note) {
		if (ClientPrefs.noteSplashes && note != null && curSong.toLowerCase() != 'parasitic') {
	...
	override function destroy() {
		...
		gfMap.clear();
		variables.clear();
		//clear anything created by lua
		modchartTweens.clear();
		modchartTimers.clear();
		modchartSounds.clear();
		modchartTexts.clear();
		modchartSaves.clear();
		modchartShaders.clear();
	...
	override function beatHit() {
		...
		for (character in modchartCharacters) {
			if (curBeat % character.danceEveryNumBeats == 0
				&& character.animation.curAnim != null
				&& !character.animation.curAnim.name.startsWith('sing')
				&& !character.stunned) {
				character.dance();
	...
	override function sectionHit() {
		...
		if (SONG.notes[curSection] != null) {
			...
			if (glitch.amount < 0.45) {
				glitch.amount += 0.35 / 3 * camZoomingMult;
			}
...
} class TutorialPromptSubstate extends MusicBeatSubstate {
	var grpOptions:FlxTypedGroup<Alphabet>;
	...
	public function new() {
		...
		var textText:Alphabet = new Alphabet(0, 0, "CHANGE OPTIONS?", true);
		textText.screenCenter(X);
		textText.y = 100;
		textText.alpha = 0;
		add(textText);
		...
		for (i in 0...["YES", "NO"].length) {
			var optionText:Alphabet = new Alphabet(0, 0, ["YES", "NO"][i], true);
			optionText.screenCenter();
			optionText.y += (100 * (i - (["YES", "NO"].length / 2))) + 50;
			optionText.alpha = 0;
			FlxTween.tween(optionText, {alpha: 1}, 0.4, {ease: FlxEase.quartInOut});
		...
		FlxTween.tween(black, {alpha: 0.6}, 0.4, {ease: FlxEase.quartInOut});
		FlxTween.tween(textText, {alpha: 1, y: 20}, 0.4, {ease: FlxEase.quartInOut, startDelay: 0.3});
		FlxTween.tween(selectorLeft, {alpha: 1}, 0.4, {ease: FlxEase.quartInOut, startDelay: 0.5});
		FlxTween.tween(selectorRight, {alpha: 1}, 0.4, {ease: FlxEase.quartInOut, startDelay: 0.7});
	...
	function changeSelection(change:Int = 0) {
		curSelected += change;
		if (curSelected < 0)
			curSelected = ["YES", "NO"].length - 1;
		if (curSelected >= ["YES", "NO"].length)
			curSelected = 0;
	...
	override function update(elapsed:Float) {
		...
		if (controls.ACCEPT && !Progress.getData("playedTutorial")) {
			Progress.setData(true, "playedTutorial");
			if (curSelected == 0) {
				FlxG.switchState(() -> new OptionsPlayState());
				FlxG.sound.play(Paths.sound('confirmMenu'));
			} else {
				trace("default binds, laugh at this user!");
