...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('luaDeprecatedWarnings', false);
		...
		Lua_helper.add_callback(lua, "initShader", function(id:String, file:String, ?glslVersion:Int = 100){
			var funnyFlxRuntimeShader:FlxRuntimeShader = new FlxRuntimeShader(File.getContent('mods/shaders/${file}.frag'), null, glslVersion);
			PlayState.instance.modchartShaders.set(id, funnyFlxRuntimeShader);
		});
		...
		Lua_helper.add_callback(lua, "setActorShader", function(actorStr:String, shaderName:String) {
			...
			var funnyFlxRuntimeShader:FlxRuntimeShader = PlayState.instance.modchartShaders.get(shaderName);
            var actor = PlayState.instance.modchartSprites.get(actorStr);
            
            if(actor != null && funnyFlxRuntimeShader != null)
            {
                actor.shader = funnyFlxRuntimeShader;
                trace('added shader '+shaderName+" to " + actorStr);
            }
        });

		Lua_helper.add_callback(lua, "setCameraShader", function(camera:String, id:String){
			if (!ClientPrefs.shaders)
                return;

			var cam = PlayState.instance.modchartCameras.get(camera);
			var funnyFlxRuntimeShader:FlxRuntimeShader = PlayState.instance.modchartShaders.get(id);
			if(cam != null && funnyFlxRuntimeShader != null){
				cam.shaders.push(new ShaderFilter(funnyFlxRuntimeShader));
		...
		Lua_helper.add_callback(lua, "setShaderProperty", function(id:String, property:String, value:Dynamic) {
			if (!ClientPrefs.shaders)
                return;
			var funnyFlxRuntimeShader:FlxRuntimeShader = PlayState.instance.modchartShaders.get(id);
			if(Std.isOfType(value, Float)){
				funnyFlxRuntimeShader.setFloat(property, Std.parseFloat(value));
			}
			else if(Std.isOfType(value, Bool)){
				funnyFlxRuntimeShader.setBool(property, value);
			}
			else{
				funnyFlxRuntimeShader.setFloatArray(property, value);
			}
		});

		Lua_helper.add_callback(lua, "getShaderFloatProperty", function(id:String, property:String) {
			return PlayState.instance.modchartShaders.get(id).getFloat(property);
		});

		Lua_helper.add_callback(lua, "tweenShaderProperty", function(id:String, property:String, value:Float, duration:Float, ?ease:String = "linear", ?startDelay:Float = 0.0, ?onComplete:Dynamic) {
			if (!ClientPrefs.shaders)
                return;
			var shader:FlxRuntimeShader = PlayState.instance.modchartShaders.get(id);
			if (shader != null) {
				shader.tween(property, value, duration, getFlxEaseByString(ease), startDelay, onComplete);
			} else {
				trace('Shader named $shader doesn\'t exist!');
			}
		});
		...
		Lua_helper.add_callback(lua, "makeLuaCharacter", function(tag:String, x:Float, y:Float, ?character:String = 'bf', ?isPlayer:Bool = false) {
			var character:Character = new Character(x, y, character, isPlayer);
			character.antialiasing = ClientPrefs.globalAntialiasing;
			character.ignorePlayerHoldChecks = true; //fix some dumb bug where extra players wont idle
			PlayState.instance.modchartCharacters.set(tag, character);
			character.active = true;
		});
		Lua_helper.add_callback(lua, "playCharacterAnim", function(obj:String, name:String, forced:Bool = false, ?reverse:Bool = false, ?startFrame:Int = 0)
			{
				if(PlayState.instance.getLuaObject(obj, false) != null) {
					var luaObj:Character = cast PlayState.instance.getLuaObject(obj,false);
		...
		Lua_helper.add_callback(lua, "makeLuaText", function(tag:String, text:String, width:Int, x:Float, y:Float) {
			...
			leText.antialiasing = ClientPrefs.globalAntialiasing;
		...
		Lua_helper.add_callback(lua, "distanceBetween", function(SpriteA:String, SpriteB:String) {
		...
		Lua_helper.add_callback(lua, "distanceToMouse", function(sprite:String) {
		...
		Lua_helper.add_callback(lua, "distanceToPoint", function(sprite:String, x:Float, y:Float) {
		...
		Lua_helper.add_callback(lua, "dotProduct", function(ax:Float, ay:Float, bx:Float, by:Float) {
            return FlxMath.dotProduct(ax, ay, bx, by);
		});

		Lua_helper.add_callback(lua, "equal", function(aValueA:Float, aValueB:Float, aDiff:Float = FlxMath.EPSILON) {
            return FlxMath.equal(aValueA, aValueB, aDiff);
		});

		Lua_helper.add_callback(lua, "fastCos", function(n:Float) {
            return FlxMath.fastCos(n);
		});

		Lua_helper.add_callback(lua, "fastSin", function(n:Float) {
            return FlxMath.fastSin(n);
		});

		Lua_helper.add_callback(lua, "fastTan", function(n:Float) {
            return FlxMath.fastSin(n) / FlxMath.fastCos(n);
		});

		Lua_helper.add_callback(lua, "fceil", function(n:Float) {
            return Math.fceil(n);
		});

		Lua_helper.add_callback(lua, "ffloor", function(n:Float) {
            return Math.ffloor(n);
		});

		Lua_helper.add_callback(lua, "fround", function(n:Float) {
            return Math.fround(n);
		});

		Lua_helper.add_callback(lua, "isFinite", function(n:Float) {
            return Math.isFinite(n);
		});

		Lua_helper.add_callback(lua, "isNaN", function(n:Float) {
            return Math.isNaN(n);
		});

		Lua_helper.add_callback(lua, "getDecimals", function(n:Float) {
            return FlxMath.getDecimals(n);
		});

		Lua_helper.add_callback(lua, "inBounds", function(value:Float, min:Null<Float>, max:Null<Float>) {
            return FlxMath.inBounds(value, min, max);
		});

		Lua_helper.add_callback(lua, "isDistanceToMouseWithin", function(sprite:String, distance:Float, includeEqual:Bool = false) {
            if(PlayState.instance.modchartSprites.get(sprite) != null)
		...
		Lua_helper.add_callback(lua, "isDistanceToPointWithin", function(sprite:String, x:Float, y:Float, distance:Float, includeEqual:Bool = false) {
			var point:FlxPoint = new FlxPoint(x, y);
            if(PlayState.instance.modchartSprites.get(sprite) != null)
		...
		Lua_helper.add_callback(lua, "isDistanceWithin", function(SpriteA:String, SpriteB:String, distance:Float, includeEqual:Bool = false) {
            if(PlayState.instance.modchartSprites.get(SpriteA) != null && PlayState.instance.modchartSprites.get(SpriteB) != null)
                return FlxMath.isDistanceWithin(PlayState.instance.modchartSprites.get(SpriteA), PlayState.instance.modchartSprites.get(SpriteB), distance, includeEqual);
            else
                return false;
		});

		Lua_helper.add_callback(lua, "isEven", function(n:Float) {
			return FlxMath.isEven(n);
		});

		Lua_helper.add_callback(lua, "isOdd", function(n:Float) {
		...
		Lua_helper.add_callback(lua, "maxAdd", function(value:Int, amount:Int, max:Int, min:Int = 1) {
			return FlxMath.maxAdd(value, amount, max, min);
		});

		Lua_helper.add_callback(lua, "maxInt", function(a:Int, b:Int) {
			return FlxMath.maxInt(a, b);
		});

		Lua_helper.add_callback(lua, "minInt", function(a:Int, b:Int) {
			return FlxMath.minInt(a, b);
		});

		Lua_helper.add_callback(lua, "numericComparison", function(a:Float, b:Float) {
			return FlxMath.numericComparison(a, b);
		});


		Lua_helper.add_callback(lua, "pointInCoordinates", function(pointX:Float, pointY:Float, rectX:Float, rectY:Float, rectWidth:Float, rectHeight:Float) {
			return FlxMath.pointInCoordinates(pointX, pointX, rectX, rectY, rectWidth, rectHeight);
		});

		Lua_helper.add_callback(lua, "random", function() {
			return Math.random();
		});

		Lua_helper.add_callback(lua, "randomBool", function(chance:Float):Bool {
			return FlxG.random.bool(chance);
		});
		...
		Lua_helper.add_callback(lua, "remapToRange", function(value:Float, start1:Float, stop1:Float, start2:Float, stop2:Float) {
			return FlxMath.remapToRange(value, start1, stop1, start2, stop2);
		});

		Lua_helper.add_callback(lua, "round", function(v:Float) {
			return Math.round(v);
		});

		Lua_helper.add_callback(lua, "roundDecimal", function(value:Float, percision:Int) {
			return FlxMath.roundDecimal(value, percision);
		});

		Lua_helper.add_callback(lua, "sameSign", function(a:Float, b:Float) {
			return FlxMath.sameSign(a, b);
		});

		Lua_helper.add_callback(lua, "signOf", function(n:Float) {
			return FlxMath.signOf(n);
		});

		Lua_helper.add_callback(lua, "sinh", function(n:Float) {
			return FlxMath.sinh(n);
		});

		Lua_helper.add_callback(lua, "vectorLength", function(dx:Float, dy:Float) {
			return FlxMath.vectorLength(dx, dy);
		});

		Lua_helper.add_callback(lua, "wrap", function(value:Int, min:Int, max:Int) {
			return FlxMath.wrap(value, min, max);
		});

		PlayState.instance.modchartCameras.set("game", {cam: PlayState.instance.camGame, shaders: [], shaderNames: []});
        PlayState.instance.modchartCameras.set("hud", {cam: PlayState.instance.camHUD, shaders: [], shaderNames: []});	
	...
	public function luaTrace(text:String, ignoreCheck:Bool = false, deprecated:Bool = false, color:FlxColor = FlxColor.WHITE) {
		...
		if(ignoreCheck || getBool('luaDebugMode')) {
			...
			PlayState.instance.addTextToDebug('$scriptName.lua: $text', color);
			trace('$scriptName.lua: $text');
		}
		#end
	}
	...
	function getCameraByName(id:String):LuaCamera{
		if(PlayState.instance.modchartCameras.exists(id))
			return PlayState.instance.modchartCameras.get(id);
	
		switch(id.toLowerCase()){
			case 'camhud' | 'hud': return PlayState.instance.modchartCameras.get("hud");
		}
			
	
		return PlayState.instance.modchartCameras.get("game");
	}
}
...
class ModchartText extends FlxText
{
	...
	public function new(x:Float, y:Float, text:String, width:Float)
	{
		...
		setFormat(Paths.font("wii.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
class DebugLuaText extends FlxText
{
	...
	public function new(text:String, parentGroup:FlxTypedGroup<DebugLuaText>, color:FlxColor) {
		...
		setFormat(Paths.font("wii.ttf"), 16, color, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scrollFactor.set();
		borderSize = 1;
	}
